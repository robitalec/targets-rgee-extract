...
R ecosystem for sampling from rasters, but Earth Engine (and its community of
users) offers an incredible resource of data sets (see below), accessible without 
downloading any large files to your local machine. 
With that in mind, this project combining `targets` and
rgee`, and a couple simple helper functions, hopes to simple operations
with Earth Engine easier. We won't cover the anywhere near the full potential
...
worked for me. You'll first need system dependencies, then some steps to install 
packages and authenticate with Earth Engine. 
